...
`;
async function atomicUpdate(id: string, operation: string, data: unknown) { const redis = redisConfig(); if (!redis) return null; const result = await redis.eval(atomicMutation, [key(id)], [operation, JSON.stringify(data), String(TTL)]); return result ? JSON.parse(result as string) : null; }
