...
export async function saveRoom(room: Room, signals: Signal[] = []) { const state = { ...room, signals }; const redis = redisConfig(); if (redis) await redis.set(key(room.id), state, { ex: TTL }); else memory.set(room.id, state); return room; }
